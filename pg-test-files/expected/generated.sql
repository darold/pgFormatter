...
CREATE TABLE gtest_err_7b (
    a int PRIMARY KEY,
    b int GENERATED ALWAYS AS (row_number() OVER (ORDER BY a)) STORED
);
...
CREATE TABLE gtest28 OF gtest_type (
    f1 WITH OPTIONS GENERATED ALWAYS AS (f2 * 2) STORED
);
...
CREATE TABLE gtest_child PARTITION OF gtest_parent (f3 WITH OPTIONS GENERATED ALWAYS AS (f2 * 2) STORED)
...
INSERT INTO gtest_parent (f1, f2)
    VALUES ('2016-07-15', 1);
...
CREATE TABLE gtest28b (
    LIKE gtest28a INCLUDING GENERATED
);
