...
CREATE FUNCTION xfloat4in (cstring)
    RETURNS xfloat4 IMMUTABLE STRICT
    LANGUAGE internal
    AS 'int4in'
;
...
select float4send(flt) as ibits,
       ...
       float4send(flt::text::float4) AS obits, float4send(flt::text::float4) = float4send(flt
) AS correct FROM (
        SELECT
            bits::integer::xfloat4::float4 AS flt FROM testdata offset 0) s;
