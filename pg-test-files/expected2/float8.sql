...
select float8send(flt) as ibits,
       ...
       float8send(flt::text::float8) AS obits, float8send(flt::text::float8) = float8send(flt
) AS correct FROM (
        SELECT
            bits::bigint::xfloat8::float8 AS flt FROM testdata offset 0) s;
