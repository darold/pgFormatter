...
5 10 20 20 30 10 50 35 80 15.CREATE FUNCTION trigger_func ()
    RETURNS TRIGGER
    LANGUAGE plpgsql
    AS '
 BEGIN
...
FROM
    stdin;
123 999 456 999. DELETE FROM main_table
WHERE a IN (123, 456);
UPDATE
    main_table
SET
...
SET
    a = a;
        ...
        COPY parted_stmt_trig (a)
    FROM
        stdin;
        1 2.
        -- insert via copy on the first partition
        COPY parted_stmt_trig1 (a)
    FROM
        stdin;
        1.
