...
=head2 Option --matching-paren-newline

For text output, when formatted content starts on a new line immediately after
an opening parenthesis, place the matching closing parenthesis on its own line.
Use the indentation of the line containing the opening parenthesis. Nested pairs
are handled independently. This option does not force opening line breaks or
change content indentation, and is disabled by default.

For example, combined with C<--comma-break>:
...
Without this option, the closing parenthesis remains beside C<label>.
An inline list such as C<INSERT INTO widgets (id, label)> stays inline.
Text following the closing parenthesis, such as C<THEN>, an alias or a comma,
continues to use the existing formatting rules. Quoted text and comments are
not treated as SQL parentheses.

Use C<matching-paren-newline=1> in a configuration file, or
C<--no-matching-paren-newline> to explicitly disable the option.
...
=head1 USING AS PRE-COMMIT HOOK
