...
of repeating the command line option. By default pgFormatter look for file
~/.pg_format but you can choose an alternate configuration file using command
...
To prevent pg_format to look at ~/.pg_format file you can use the command
line option -X | --no-rcfile
