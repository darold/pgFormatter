...
GROUP BY
    ...
    3
ORDER BY
    1 \crosstabview v "month name" 4 num
...
ORDER BY
    month \crosstabview """month"" name" year format year
...
SELECT
    ...
    h,
    string_agg(c, E'\n') AS c,
    row_number() OVER (ORDER BY h) AS r
FROM
    ctv_data
GROUP BY
    v,
...
SELECT
    ...
    h,
    string_agg(c, E'\n') AS c,
    row_number() OVER (ORDER BY h NULLS LAST) AS r
FROM
    ctv_data
GROUP BY
    v,
...
SELECT
    ...
    i
FROM
    ctv_data \crosstabview v h j

-- error: need to quote name
SELECT
    1 AS "22",
    2 AS b,
    3 AS "Foo" \crosstabview 1 2 Foo

-- error: need to not quote name
SELECT
    1 AS "22",
    2 AS b,
    3 AS "Foo" \crosstabview 1 "B" "Foo"
...
SELECT
    ...
    1
FROM
    generate_series(1, 3000) AS a \crosstabview
