...
SELECT
    3 AS x,
    4 AS y \gset pref01_ \echo :pref01_x \echo :pref01_y

SELECT
    5 AS x,
    6 AS y \gset pref01_ 

\g \echo :pref01_x :pref01_y

SELECT
    7 AS x,
    8 AS y \g \gset pref01_ \echo :pref01_x :pref01_y
...
prepare q as
  select ' SOME text ' as "a\title", E' < foo > n < bar > ' as "junk",
         ' ' as "empty", n as int
  from generate_series(1,2) as n;
...
\else
  \echo ' # 10 - 2 ok,
    variable no_such_variable IS NOT defined '
\endif
...
\echo ' number OF rows: ' :ROW_COUNT
\echo ' LAST error message: ' :LAST_ERROR_MESSAGE
\echo ' LAST error code: :LAST_ERROR_SQLSTATE \unset FETCH_COUNT
